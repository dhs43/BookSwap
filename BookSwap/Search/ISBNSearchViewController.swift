...
//  BookSwap
//
//  Created by David Shapiro on 8/8/18.
...
class ISBNSearchViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
        ...
        if metadataObjects.count != 0
        {
            if let object = metadataObjects[0] as? AVMetadataMachineReadableCodeObject
            {
                if object.type == AVMetadataObject.ObjectType.ean13
                {
                    ...
                    performSegue(withIdentifier: "searchListingsISBN", sender: self)
                    session.stopRunning()
                }
            }
        }
